...
function getMarketStartingSoonReason(market) {
  const info = getKnownMarketStartInfo(market);
  if (!info) return null;
  const closeAt = new Date(info.startsAt.getTime() - EXPIRE_BEFORE_START_MS);
...
async function placeSellLimit(market, tokenId, priceWei, quantityWei, expiresAt) {
  const builder = await initSDK();
...
async function getBestAsk(marketId, market, outcome) {
  try {
    const book = await getPredictBook(marketId);
    return getBestPredictAskLevelFromBook(book, market, outcome)?.price ?? null;
  } catch (e) {
...
function getPositionOutcome(market, pos, tokenId, outcomeId) {
  ...
  return outcomes.find(outcome => String(outcome?.onChainId) === String(tokenId))
    ?? outcomes.find(outcome => outcomeId && String(outcome?.id) === String(outcomeId))
    ?? pos?.outcome
    ?? null;
}
...
async function closeSinglePosition(pos, openOrders) {
  ...
  try {
    ...
    if (!bestAsk) {
      console.log("⚠️ 当前市场卖一为空，放弃平仓 marketId=" + marketId + " tokenId=" + tokenId);
      return;
    }
...
async function monitorSingleOrder(order, openOrders, predictBidCache) {
  ...
  try {
    ...
    if (startingSoonReason) {
      logCancelCondition("getMarketStartingSoonReason(market)", "marketId=" + marketId + " " + startingSoonReason + " title=" + marketTitle);
      await cancelOrder(orderId, startingSoonReason + " marketId=" + marketId + " title=" + marketTitle);
      return;
    }
    ...
    if (!orderPrice) {
      ...
      await cancelOrder(orderId, "买单价格缺失 marketId=" + marketId + " outcome=" + outcome.name + " predictBid=" + predictBid + " predictAsk=" + predictAsk + " title=" + marketTitle);
      return;
    }
    ...
    if (!predictAsk) {
      ...
      await cancelOrder(orderId, "卖一为空，成交后无法按卖一平仓 marketId=" + marketId + " outcome=" + outcome.name + " orderPrice=" + orderPrice + " predictBid=" + predictBid + " ask=null title=" + marketTitle);
      return;
    }

    if (predictBid && Number(predictBid) - orderPrice > PRICE_TOLERANCE) {
    ...
    if (!predictBid || polyQuote.price + PRICE_TOLERANCE < Number(predictBid)) {
      const diff = predictBid ? polyQuote.price - Number(predictBid) : null;
      logCancelCondition("!predictBid || polyQuote.price + PRICE_TOLERANCE < Number(predictBid)", "marketId=" + marketId + " outcome=" + outcome.name + " predictBid=" + predictBid + " polyBid=" + polyQuote.price + " diff=" + diff + " tolerance=" + PRICE_TOLERANCE + " polyUsd=" + polyQuote.valueUsd.toFixed(2) + " title=" + marketTitle);
      await cancelOrder(orderId, "PM优势不足 marketId=" + marketId + " outcome=" + outcome.name + " predictBid=" + predictBid + " polyBid=" + polyQuote.price + " diff=" + diff + " tolerance=" + PRICE_TOLERANCE + " polyUsd=" + polyQuote.valueUsd.toFixed(2) + " title=" + marketTitle);
      return;
    }
...
async function processMarket(market, amountWei, existingOrders) {
  ...
  if (getMarketStartingSoonReason(market)) {
    return { new: 0, skip: 1 };
  }
  ...
  for (const outcome of market.outcomes) {
    ...
    const predictBidLevel = getBestPredictBidLevelFromBook(predictBook, market, outcome);
    ...
    const price = predictBidLevel.price;
    ...
    if (predictAskLevel.price <= buyPrice + 1e-9) {
      skipOrders++;
      continue;
    }
